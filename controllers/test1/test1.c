...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if (sensor_values[RIGHT_SENSOR] > 80 || sensor_values[RIGHT_FRONT_SENSOR] > 80) {
    ...
    } else if (sensor_values[LEFT_SENSOR] > 60 && sensor_values[RIGHT_SENSOR] > 1000) {
      // Turn around if there's a wall in front
      printf("TICK\n");
    }

    printf("%g -- %g * %g -- %g \n", sensor_values[LEFT_FRONT_SENSOR], sensor_values[LEFT_SENSOR], sensor_values[RIGHT_SENSOR], sensor_values[RIGHT_FRONT_SENSOR]);
    // Set motor speeds
